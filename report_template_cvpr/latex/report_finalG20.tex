...
The network can achieve this splitting the image into a fixed size grid, analyzing each cell to find
correspondeces between objects in cell and ground truth objects. The net tries to fits objects into
fixed size rectangles called {\tt Anchors}, each one with different ratios and scales.
YOLO networks returns a prediction vector with shape: {\tt [$p_c$, $b_{cx}$, $b_{cy}$, $b_w$, $b_h$, $c_1$, ..., $c_N$]} 
where:
\begin{itemize}
  \item $p_c$ is the probability that there is an object belonging to one of the classes
  \item $b_{cx}$ is the x center coordinate of the bounding box
  \item $b_{cy}$ is the y center coordinate of the bounding box
  \item $b_w$ is the width of the bounding box
  \item $b_h$ is the height of the bounding box
  \item $c_1$ ... $c_N$ are the probabilities of belonging to each class
\end{itemize}
